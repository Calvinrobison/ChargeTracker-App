...
| I0 | **The installer builds and passes verify:package, then crashes on install** | — | Exit `-1073740940` (`STATUS_HEAP_CORRUPTION`) during a silent install of the 328 MB package. **Cause established:** electron-builder's bundled `7za.exe` is 32-bit and cannot address the ~700 MB of LZMA encoder memory `-mx=9` needs on an 814 MiB payload. With `differentialPackage` off the same fault surfaced at build time as `Can't allocate required memory!` instead of emitting a damaged installer. `build-all.ps1` now puts a 64-bit 7-Zip on `PATH`. **Still blocks the release until a build with that fix installs and self-checks green** — the diagnosis is sound but unverified. |
