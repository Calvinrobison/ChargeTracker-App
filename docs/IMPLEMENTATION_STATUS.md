...
| I0g    | ~~Every collection run was discarded~~                                        | —          | **Fixed in 0.3.1.** `port_observations.port_id` is `NOT NULL REFERENCES ports (id)`, foreign keys are ON, the collector synthesises a port id, and nothing had ever inserted a `ports` row. The FK rejection aborted the run transaction, so 0.3.0 collected every 30s and stored nothing. Ingest now upserts the parent row on `(scope_key, source_port_id)`. Passed 507 specs, 48 UI specs and a 16/16 self-check, because none performed a real ingest carrying ports; the spec that appeared to cover it inserted the `ports` row itself. |
| I0h    | `ports.retired` is written by nothing                                         | —          | A port that disappears from a source stays marked active for ever. Retiring one needs a policy — how many absences count, and what retirement does to per-port history and to metrics — so it is a behaviour decision, not part of repairing a foreign key. Recorded rather than guessed at while fixing I0g.                                                                                                                                                                                                                                  |
| I0i    | `ports.reported_power_kw` has no source                                       | —          | Nothing in the collector, domain or shared layer captures power, so there is no value to write and NULL is accurate. Listed so the empty column is not mistaken for a defect, and so that adding power to the adapter is known to have a column waiting.                                                                                                                                                                                                                                                                                      |
| I0j    | `connectors`, `inferred_episodes` and `hourly_metrics` are never written      | —          | Declared in the schema and populated by nothing. Unlike `ports` they sit on no write path, so they cannot abort a run; `hourly_metrics` is read only for a staleness count, which reads 0. Each represents a planned feature, and each is a place where a future write will need its parent rows to exist.                                                                                                                                                                                                                                     |
