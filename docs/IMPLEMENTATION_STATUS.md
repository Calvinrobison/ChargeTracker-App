...
**Schema version**: 1
**Test count**: 329 specs, all passing, via `npm run test:nodeps`
...
made the two highest-risk areas — the metric contract and the real SQL schema —
genuinely testable with zero dependencies, along with the scheduler, the source
...
Every item below is covered by specs that run via `npm run test:nodeps`.
...
Files: `src/database/`, `tests/nodeps/{database,csv-backup,queries}.test.ts`.
...
them needs dependencies this environment could not install. They are **not**
claimed as working.

- `src/collector/browser.ts` — bundled Chromium runtime, page pool, readiness,
  shutdown of only our own child processes.
- `src/collector/adapters/chargepoint/index.ts` — the Playwright wiring around
  the tested parser.
- `src/collector/service.ts` — orchestration, provenance envelopes, graceful stop.
- `src/database/worker.ts` — smoke-run successfully against `node:sqlite`
  (opens, migrates with WAL, seeds settings, exports CSV, correctly refuses a
  backup on a driver without online-backup support), but never run under
  Electron with better-sqlite3.
- `src/workers/{database,collector}.ts` — utility-process entry points.
- `src/main/{workers,updates}.ts` — worker supervision and the UpdateService.
- `src/preload/index.ts` — the narrow bridge.
...
| B2 | Live ChargePoint collection | No route to `driver.chargepoint.com` **or** to its terms pages. Adapter ships `eligibilityState: needs_review`, `verificationState: blocked` | A terms review with recorded scope, then a bounded live read (docs/SOURCE_VERIFICATION.md) |
| B3 | Bundled Mesa station catalog | No route to `afdc.energy.gov`. **No catalog data is shipped** — inventing station rows was not an option | `npm run catalog:refresh` once written (I3), on a networked machine |
| B4 | Windows installer, bundled Chromium, native SQLite rebuild | No Windows host | `npm run package:win` on Windows 11 x64 once I4 is written |
| B5 | A→B installed update proof | Same as B4 | `npm run test:update` once I5 is written |
| B6 | Push to GitHub | Git proxy: repository not in the session's authorized set | Attach the repo with push access, or push from your own machine (commands in docs/HANDOFF.md) |
...
## Outstanding implementation, in the order I would do it

| # | Work | Why this order |
| --- | --- | --- |
| I1 | `src/main/index.ts`, `window.ts`, `tray.ts` — app lifecycle, single-instance lock, tray, power-monitor wiring, `updates-backend.ts` for the pinned electron-updater | Nothing runs without an entry point. `package.json` points at `out/main/index.js`, which does not exist yet. |
| I2 | The renderer: Overview, Map, station drawer, onboarding, settings | The design handoff is detailed and complete; build it against the finished IPC contract and view models. `docs/UI_HANDOFF.md` has the tokens and layout. |
| I3 | `scripts/`: `catalog-refresh`, `setup-browser`, `keys-bootstrap`, `verify-package`, `release-{prepare,verify,publish}` | Needed before any packaging or release can be honest. |
| I4 | `electron.vite.config.ts`, `electron-builder.yml`, packaging | Depends on I1/I2 existing. |
| I5 | `scripts/windows/test-{installed,update}.ps1` and the two-version harness | Depends on I4. |
| I6 | CI (`ci.yml`, `release.yml`), issue/PR templates, dependabot | Depends on the lockfile (B1). |
| I7 | Vitest projects wrapping the no-deps specs plus dependency-requiring tests, and Playwright UI tests | Depends on B1. |

Scripts referenced in docs but deliberately **absent** from `package.json`
rather than present as stubs that would report success without doing anything:
`dev`, `build`, `package:win`, `verify:package`, `setup:browser`,
`catalog:refresh`, `keys:bootstrap`, `release:prepare`, `release:verify`,
`release:publish`, `test:installed`, `test:update`.
...
```powershell
npm install                # resolves versions; see docs/BUILDING.md first
...
See [VERIFICATION_REPORT.md](VERIFICATION_REPORT.md) for the item-by-item
table with pass / fail / skip / not-tested for each of the 23 criteria.
Summary: **9 pass, 0 fail, 14 not-tested or blocked.**
