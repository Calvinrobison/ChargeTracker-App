...
| I0b | Packaging fails with `Can't allocate required memory!` | — | **Resolved, unverified.** Turning off `differentialPackage` removed the 1 MB dictionary it imposes, restoring the `-mx=9` default of 64 MB — ~700 MB of encoder memory, which electron-builder's 32-bit bundled `7za.exe` cannot address. `USE_SYSTEM_7ZA` no longer exists in electron-builder 26; `build-all.ps1` now sets `ELECTRON_BUILDER_7ZIP_PATH` to a system 64-bit 7-Zip, falling back to compression level 5. Introduced by the fix for I0 and not present before it. |
