...
| I0b | Packaging fails with `Can't allocate required memory!` | — | **Unverified fix, second attempt.** Turning off `differentialPackage` removed the 1 MB dictionary it imposes, restoring `-mx=9`: a 64 MB dictionary held per encoder thread. First blamed on the bundled 32-bit `7za.exe`; a 64-bit 7-Zip 26.03 then failed identically, so architecture was never the constraint. `build-all.ps1` now sets `ELECTRON_BUILDER_COMPRESSION_LEVEL=5` and reports cores and free memory. Introduced by the fix for I0. |
| I0c | The package ships a second Chromium | — | 432 MB of the 814 MiB payload is a full `chrome-win64` beside the Chromium already inside Electron. It is the direct cause of I0b and most of the installer's size, and it is a design question, not a build-script one: either drive collection through Electron's own browser, or fetch the collector browser on first run. Neither is attempted yet. |
