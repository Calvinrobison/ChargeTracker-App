...
No source is cleared, so no observations are ever recorded and every occupancy
number stays empty regardless of the catalog. `docs/SOURCE_VERIFICATION.md`
sets out what a review has to establish. This is a legal and product judgement,
not an engineering task, and the answer may legitimately be "ChargePoint is
ineligible" — which is a real outcome, not a failure. Until it is answered the
app can show a map of stations and nothing about how busy they are.
