...
| 14 | Full Windows installer with working browser, native SQLite and assets | **F** | **The installer now BUILDS and passes `verify:package` 10/10 — and then fails to install.** `ChargeWatch-Setup-0.1.0.exe` (328 MB) exits with `-1073740940` (`STATUS_HEAP_CORRUPTION`) during a silent install; the NSIS process crashes rather than reporting an error. **Cause now established:** electron-builder bundles a 32-bit `7za.exe`, which cannot address the ~700 MB of LZMA encoder memory `-mx=9` needs on an 814 MiB payload. A later build failed loudly at the same point with `Can't allocate required memory!`, naming `7-Zip (a) 24.09 (x86)`. `build-all.ps1` now puts a 64-bit 7-Zip on `PATH`. Still recorded as **F**, not P: the diagnosis explains both failures but no installer built with the fix has yet been run. |
